...
export function handleBetCreated(event: BetCreated): void {
  ...
  bet.priceExponent = 0;
  bet.priceFeedId = Bytes.fromHexString("0x00");
  bet.assetPair = "ETH/USD"; // Default to ETH/USD
  ...
  globalStats.totalActiveBets = globalStats.totalActiveBets.plus(BigInt.fromI32(1));
...
export function handleBetResolved(event: BetResolved): void {
  ...
  // Update global stats
  let globalStats = getOrCreateGlobalStats();
  globalStats.totalActiveBets = globalStats.totalActiveBets.minus(BigInt.fromI32(1));
  globalStats.totalResolvedBets = globalStats.totalResolvedBets.plus(BigInt.fromI32(1));
  globalStats.save();
...
export function handleBetCancelled(event: BetCancelled): void {
  ...
  let globalStats = getOrCreateGlobalStats();
  globalStats.totalActiveBets = globalStats.totalActiveBets.minus(BigInt.fromI32(1));
  globalStats.totalCancelledBets = globalStats.totalCancelledBets.plus(BigInt.fromI32(1));
  globalStats.save();
}